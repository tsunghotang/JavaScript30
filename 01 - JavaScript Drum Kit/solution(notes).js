...
// capture all the elements with a class of 'key' and for each element add an event listener that listens for the transition end event
...
keys.forEach(key => { // iterate over each element and add an eventListener to each
  key.addEventListener('transitionend', removeTransition); //The transition end event fires when an animation ends
})
